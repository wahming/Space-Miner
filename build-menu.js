	...
	graphicalInit: function () {
		...
		for (var i = 0; i < buildings.length; i++) {
			if (buildings[i].appear_in_game) {
				$buildMenu.append(this.addToMenu.call(buildings[i]));
			}
		}
	},
				...
				if (buildings[i].appear_in_game) {
					...
					if (buildings[i].cost.length > 0) {
						...
						for (j = 0; j < buildings[i].cost.length; j++) {
							var item = SpaceMiner.General.findObject(buildings[i].cost[j][0]);
	...
	addToMenu: function () {
		...
		if (this.production.length > 0) {
			for (i in this.production) {
				...
				produces += this.production[i][1] + " " + SpaceMiner.General.findObject(this.production[i][0]).name;
			}
		} else {
			produces += "-";
		}
		...
		if (this.consumption.length > 0) {
			for (i in this.consumption) {
				...
				consumes += this.consumption[i][1] + " " + SpaceMiner.General.findObject(this.consumption[i][0]).name;
			}
		} else {
			consumes += "-";
		}
		...
		if (this.cost.length > 0) {
			for (i in this.cost) {
				...
				cost += (this.cost[i][1]) + " " + SpaceMiner.General.findObject(this.cost[i][0]).name;
			}
		} else {
			cost += "-";
		}
		...
		if (this.storage.length > 0) {
			for (i in this.storage) {
				...
				storage += this.storage[i][1] + " " + SpaceMiner.General.findObject(this.storage[i][0]).name;
			}
		} else {
			storage += "-";
		}
		...
		if (this.buildable) {
			...
			(function (item) {
				$('#' + div_name + "-build-button").on('click', function () {
					SpaceMiner.General.create.call(item);
				});
			}(this));
